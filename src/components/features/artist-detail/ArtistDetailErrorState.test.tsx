...
describe('ArtistDetailErrorState', () => {
  ...
  it('renders with default props', () => {
    ...
    expect(screen.getByText('Erro ao carregar dados do artista')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Voltar para Artistas' })).toBeInTheDocument();
  });
  ...
  it('displays custom error message', () => {
    ...
    render(<ArtistDetailErrorState {...defaultProps} errorMessage={customMessage} />);
  ...
  it('calls onBackClick when back button is clicked', () => {
    ...
    render(<ArtistDetailErrorState {...defaultProps} onBackClick={mockOnBackClick} />);
    
    const backButton = screen.getByRole('button', { name: 'Voltar para Artistas' });
  ...
  it('has correct styling classes', () => {
    ...
    const container = screen.getByText('Erro ao carregar dados do artista').closest('div');
    expect(container?.parentElement).toHaveClass('flex', 'flex-col', 'items-center', 'justify-center', 'min-h-screen');
    
    // Verifica o botão
    const button = screen.getByRole('button', { name: 'Voltar para Artistas' });
    expect(button).toHaveClass('px-4', 'py-2', 'bg-green-500', 'text-white', 'rounded-lg', 'hover:bg-green-600', 'transition-colors');
  });
  ...
  it('renders with offline state and appropriate styling', () => {
    const offlineMessage = 'Você está offline. Verifique sua conexão com a internet.';
    render(
  ...
  it('handles long error messages', () => {
    const longMessage = 'Este é um erro muito longo que pode acontecer quando há problemas de conectividade ou quando o servidor está indisponível por um período prolongado de tempo';
    render(<ArtistDetailErrorState {...defaultProps} errorMessage={longMessage} />);
    
    expect(screen.getByText(longMessage)).toBeInTheDocument();
  });

  it('handles empty error message', () => {
    const { container } = render(<ArtistDetailErrorState {...defaultProps} errorMessage="" />);
  ...
  it('has proper component structure', () => {
    ...
    const mainContainer = screen.getByText('Erro ao carregar dados do artista').closest('div')?.parentElement;
    expect(mainContainer).toBeInTheDocument();
    
    // Verifica se existe um container para o ícone e mensagem
    const iconMessageContainer = screen.getByText('Erro ao carregar dados do artista').closest('div');
