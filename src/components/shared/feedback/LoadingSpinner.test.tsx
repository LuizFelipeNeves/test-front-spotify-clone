...
describe('LoadingSpinner', () => {
  ...
  it('has correct styling classes', () => {
    ...
    expect(container).toHaveClass('flex', 'flex-col', 'items-center', 'justify-center', 'py-8', 'px-4');
    
    const spinnerDiv = document.querySelector('.animate-spin');
    expect(spinnerDiv).toHaveClass('border-2', 'border-gray-600', 'border-t-green-500', 'rounded-full', 'animate-spin', 'mb-4');
  ...
  it('renders with all size and message combinations', () => {
    ...
    sizes.forEach(size => {
      const { unmount } = render(<LoadingSpinner size={size} message={customMessage} />);
