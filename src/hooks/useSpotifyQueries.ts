...
export const spotifyQueryKeys = {
  topArtists: (limit?: number, offset?: number) => ['topArtists', limit, offset] as const,
...
export function useInfiniteTopArtists(limit = 20) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => spotifyService.getTopArtists(limit, pageParam),
...
export function useInfiniteUserPlaylists(limit = 50) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => spotifyService.getUserPlaylists(limit, pageParam),
    getNextPageParam: (lastPage, allPages) => {
      const nextOffset = allPages.length * limit
      return lastPage.next ? nextOffset : undefined
    },
...
export function useInfiniteArtistAlbums(artistId: string, limit = 20) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => spotifyService.getArtistAlbums(artistId, limit, pageParam),
    getNextPageParam: (lastPage, allPages) => {
      const nextOffset = allPages.length * limit
      return lastPage.next ? nextOffset : undefined
    },
